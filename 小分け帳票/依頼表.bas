...
'    End If
    ...
    Worksheets("小分け依頼表").Activate
    work_date = Day(Worksheets("小分け依頼表").Range("O1"))
    work_date_2 = Day(DateAdd("d", 1, Worksheets("小分け依頼表").Range("O1")))
    ...
    For i = 1 To UBound(sheetData) - 10
        ...
        If sheetData(i, 5) = "指示数" And sheetData(i, work_date + 7) <> Empty Then
            For s = 0 To 4
                paste_data(add_No, 0) = Worksheets("小分け依頼表").Range("O1")
...
Continue:
    ...
    For i = 1 To UBound(sheetData) - 10
        ...
        If sheetData(i, 5) = "指示数" And sheetData(i, work_date_2 + 7) <> Empty Then
            For s = 0 To 4
                paste_data(add_No, 0) = DateAdd("d", 1, Worksheets("小分け依頼表").Range("O1"))
                paste_data(add_No, 1) = sheetData(i + s, 2)
                If s = 0 Then
...
Continue2:
    ...
    Worksheets("小分け依頼表").Range("C6:D760").ClearContents   'C列D列　日付け、部品
    Worksheets("小分け依頼表").Range("J6:J760").ClearContents   'J列　指示数
    Worksheets("小分け依頼表").Range("L6:L760").ClearContents   'L列　順番
    ...
    Worksheets("小分け依頼表").Range(Cells(6, 3), Cells(UBound(C_paste) + 5, 3)) = C_paste  '日付け
    Worksheets("小分け依頼表").Range(Cells(6, 4), Cells(UBound(C_paste) + 5, 4)) = D_paste  '部品
    Worksheets("小分け依頼表").Range(Cells(6, 10), Cells(UBound(C_paste) + 5, 10)) = J_paste    '指示数
...
Sub 依頼表並び替え()
    ...
    If Worksheets("小分け依頼表").Range("T1") = "X" Or Worksheets("小分け依頼表").Range("T1") = "エラー" Then
    ...
    L_order_col = Worksheets("小分け依頼表").Range(Cells(6, 12), Cells(700, 12))   'L列
    D_parts_col = Worksheets("小分け依頼表").Range(Cells(6, 4), Cells(700, 4))    'D列
    ...
    If ActiveSheet.FilterMode Then ActiveSheet.ShowAllData  'フィルター解除
    Worksheets("小分け依頼表").Range("L6:L760").ClearContents    'L列削除
    Worksheets("小分け依頼表").Range(Cells(6, 12), Cells(700, 12)) = L_order_col     'L列貼付け

    '*********************L列ソート************************************************
    date_col = Worksheets("小分け依頼表").Range(Cells(6, 3), Cells(700, 3))   'C列シートデータ取得
    parts_col = Worksheets("小分け依頼表").Range(Cells(6, 4), Cells(700, 4))   'D列シートデータ取得
    shiji_col = Worksheets("小分け依頼表").Range(Cells(6, 10), Cells(700, 10))   'J列シートデータ取得
    order_col = Worksheets("小分け依頼表").Range(Cells(6, 12), Cells(700, 12))   'L列シートデータ取得
    ...
    Call クイックソート(order_sort, LBound(order_sort), UBound(order_sort))  '戻り値ないけどorder_sortは変わる
   
   '*********************貼り付けデータの計算************************************************
    Dim paste_data() As Variant         '貼付け
    ReDim paste_data(700, 3)
    ...
    Worksheets("小分け依頼表").Range(Cells(6, 3), Cells(700, 3)) = C_paste 'C列シートデータ貼付
    Worksheets("小分け依頼表").Range(Cells(6, 4), Cells(700, 4)) = D_paste  'D列シートデータ貼付
    Worksheets("小分け依頼表").Range(Cells(6, 10), Cells(700, 10)) = J_paste  'J列シートデータ貼付
    Worksheets("小分け依頼表").Range(Cells(6, 12), Cells(700, 12)) = L_paste  'L列シートデータ貼付
