...
'    End If
    ...
    Worksheets("小分け依頼表_木").Activate
    work_date = Day(Worksheets("小分け依頼表_木").Range("O1"))
    work_date_2 = Day(DateAdd("d", 1, Worksheets("小分け依頼表_木").Range("O1")))
    ...
    For i = 1 To UBound(sheetData) - 10
        ...
        If sheetData(i, 5) = "指示数" And sheetData(i, work_date + 7) <> Empty Then
            For s = 0 To 4
                paste_data(add_No, 0) = Worksheets("小分け依頼表_木").Range("O1")
...
'            For s = 0 To 4
'                paste_data(add_No, 0) = DateAdd("d", 1, Worksheets("小分け依頼表_木").Range("O1"))
'                paste_data(add_No, 1) = sheetData(i + s, 2)
'                If s = 0 Then
...
'    Next i
    ...
    Worksheets("小分け依頼表_木").Range("C6:D760").ClearContents   'C列D列　日付け、部品
    Worksheets("小分け依頼表_木").Range("J6:J760").ClearContents   'J列　指示数
    Worksheets("小分け依頼表_木").Range("L6:L760").ClearContents   'L列　順番
    ...
    Worksheets("小分け依頼表_木").Range(Cells(6, 3), Cells(UBound(C_paste) + 5, 3)) = C_paste  '日付け
    Worksheets("小分け依頼表_木").Range(Cells(6, 4), Cells(UBound(C_paste) + 5, 4)) = D_paste  '部品
    Worksheets("小分け依頼表_木").Range(Cells(6, 10), Cells(UBound(C_paste) + 5, 10)) = J_paste    '指示数
...
Sub 依頼表並び替え_木()
    ...
    If Worksheets("小分け依頼表_木").Range("T1") = "X" Or Worksheets("小分け依頼表_木").Range("T1") = "エラー" Then
    ...
    L_order_col = Worksheets("小分け依頼表_木").Range(Cells(6, 12), Cells(700, 12))   'L列
    D_parts_col = Worksheets("小分け依頼表_木").Range(Cells(6, 4), Cells(700, 4))    'D列
    ...
    If ActiveSheet.FilterMode Then ActiveSheet.ShowAllData  'フィルター解除
    Worksheets("小分け依頼表_木").Range("L6:L760").ClearContents    'L列削除
    Worksheets("小分け依頼表_木").Range(Cells(6, 12), Cells(700, 12)) = L_order_col     'L列貼付け
    
    '*********************L列ソート************************************************
    date_col = Worksheets("小分け依頼表_木").Range(Cells(6, 3), Cells(700, 3))   'C列シートデータ取得
    parts_col = Worksheets("小分け依頼表_木").Range(Cells(6, 4), Cells(700, 4))   'D列シートデータ取得
    shiji_col = Worksheets("小分け依頼表_木").Range(Cells(6, 10), Cells(700, 10))   'J列シートデータ取得
    order_col = Worksheets("小分け依頼表_木").Range(Cells(6, 12), Cells(700, 12))   'L列シートデータ取得
    black_col = Worksheets("小分け依頼表_木").Range(Cells(6, 13), Cells(700, 13))   'M列シートデータ取得
    ...
    Call クイックソート_木(order_sort, LBound(order_sort), UBound(order_sort))  '戻り値ないけどorder_sortは変わる
   ...
   '*********************貼り付けデータの計算************************************************
    ...
    For e = 0 To UBound(order_sort)
        For i = 1 To UBound(order_col) Step 5
            If order_col(i, 1) = order_sort(e) Then
                ...
                paste_data(paste_No, 4) = black_col(i, 1)            '白黒
    ...
    Worksheets("小分け依頼表_木").Range(Cells(6, 3), Cells(700, 3)) = C_paste 'C列シートデータ貼付
    Worksheets("小分け依頼表_木").Range(Cells(6, 4), Cells(700, 4)) = D_paste  'D列シートデータ貼付
    Worksheets("小分け依頼表_木").Range(Cells(6, 10), Cells(700, 10)) = J_paste  'J列シートデータ貼付
    Worksheets("小分け依頼表_木").Range(Cells(6, 12), Cells(700, 12)) = L_paste  'L列シートデータ貼付
    Worksheets("小分け依頼表_木").Range(Cells(6, 13), Cells(700, 13)) = M_paste  'M列シートデータ貼付
...
Sub クイックソート_木(ByRef argAry() As Variant, _
    ...
    Loop
    If (lngMin < i - 1) Then
        Call クイックソート_木(argAry, lngMin, i - 1)
    End If
    If (lngMax > j + 1) Then
        Call クイックソート_木(argAry, j + 1, lngMax)
    End If
End Sub
