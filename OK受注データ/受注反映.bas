...
Sub 受注データ反映main()
    ...
    pData = Worksheets("ピッキング表").Range(Cells(3, 6), Cells(38, 14))   'F3からL38
    ...
    For p = 1 To UBound(kData)
        For r = 1 To 9  'F3 から N3　　列
            If pData(1, r) = kData(p, 4) Then   'センター一致
                For s = 6 To 36 'F8からF38   行
                    If Not IsEmpty(pData(s, 9)) Then
                        If pData(s, 9) <> "" Then
                            If str(pData(s, 9)) = str(kData(p, 2)) Then   '商品コード一致
                                pData(s, r) = kData(p, 6)
                            End If
                        End If
                    End If
    ...
    ReDim paste_data(30, 0)
    For i = 0 To UBound(paste_data)
        paste_data(i, 0) = pData(i + 6, 5)
    Next i
    
    '貼り付け
    Worksheets("ピッキング表").Range(Cells(8, 10), Cells(38, 10)) = paste_data

    '******************厚木
    ReDim paste_data(30, 0)
    For i = 0 To UBound(paste_data)
        paste_data(i, 0) = pData(i + 6, 7)
    Next i
    
    '貼り付け
    Worksheets("ピッキング表").Range(Cells(8, 12), Cells(38, 12)) = paste_data
