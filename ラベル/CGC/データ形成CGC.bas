...
'センターコードを判定。６行目を６列目～read_Colまで回す。
    For k = 6 To read_Col
        If IsNumeric(sheetData(6, k)) And Len(sheetData(6, k)) = 4 And sheetData(3, k) <> "" And sheetData(3, k) <> 0 Then
        ...
        For i = 1 To B_LastRow
            ...
            If IsNumeric(sheetData(i, 2)) And sheetData(i, 2) <> "" And sheetData(i, 3) <> 0 And sheetData(i, 3) <> "" And sheetData(i, k) <> 0 And sheetData(i, k) <> "" Then
                ...
                If (sheetData(i, k) / sheetData(i, 5)) <= 1 Then
                        ...
                        paste_data(dataNo, 8) = sheetData(6, 4)    'センター納品日
                        dataNo = dataNo + 1
                Else
...
Continue:             ' GoTo Continue の後はここから処理が行われる
    ...
    Worksheets("形成").Range(Cells(1, 1), Cells(1000, 10)) = paste_data
