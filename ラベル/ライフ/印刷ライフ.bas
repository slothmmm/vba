Sub ラベル印刷と計算()
    ...
    rc = MsgBox("データ更新後、印刷を行います。IP68で印刷設定していますか？", vbYesNo + vbQuestion)
    If rc = vbNo Then
...
Sub ラベル印刷()
    ...
    Dim rc As VbMsgBoxResult
    rc = MsgBox("印刷を行います。IP68で印刷設定していますか？", vbYesNo + vbQuestion)
    If rc = vbNo Then
        MsgBox "印刷を中止します", vbCritical
        Exit Sub
    End If
    ...
    If myPrinter Like "*68*" Then
    Application.ActivePrinter = myPrinter
    Else
        MsgBox myPrinter & "が選択されています。" & vbCrLf & "プリンターの設定をIP68へ変更して下さい。"
        Exit Sub
    End If
