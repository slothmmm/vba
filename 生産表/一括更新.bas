...
Sub 一括更新_CGC()
    Customer_name = "CGC"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
    Call 一括更新_フレッセイ
End Sub

Sub 一括更新_CGC北陸()
    Customer_name = "CGC北陸"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_イトーヨーカドー()
    Customer_name = "イトーヨーカドー"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_オーケー()
    Customer_name = "オーケー"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub
...
Sub 一括更新_コープデリフローズン()
    Customer_name = "コープデリフローズン"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_セイミヤ()
    Customer_name = "セイミヤ"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_ニュークイック()
    Customer_name = "ニュー･クイック"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_フレッセイ()
    Customer_name = "フレッセイ"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_ヤオコー()
    Customer_name = "ヤオコー"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_ユーコープ()
    Customer_name = "ユーコープ"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_ライフ()
    Customer_name = "ライフ"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_原信()
    Customer_name = "原信"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_東武ストア()
    Customer_name = "東武ストア"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_イクタツ()
    Customer_name = "イクタツ"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub

Sub 一括更新_SMコープデリ()
    Customer_name = "SMコープデリ"                       '出荷先
    Call 一括生産表更新main(Customer_name)
    'Call データバックアップ(Customer_name)
End Sub
...
Function 一括生産表更新main(Customer_name As Variant)
    ...
    csv_data = 一括csv読み込み(Customer_name)       '該当のcsvデータ
    ship_date = Sheets("受注入力").Range("A1")  '出荷日
    
    If IsEmpty(csv_data) Then
        Exit Function
    End If
...
Function 一括csv読み込み(Customer_name As Variant) As Variant
  ...
  file = 一括csvファイル名探索(Customer_name)
...
Function 一括csvファイル名探索(Customer_name As Variant) As Variant
    ...
    一括csvファイル名探索 = csvFilePath & "\" & filename(Max, 0)
...
Function 一括データバックアップ(Customer_name As Variant)
    file_pass_to = 一括ディレクトリ作成コピー先()
...
Function 一括ディレクトリ作成コピー先() As Variant
    Dim root As String
    Dim yyyy As String
    Dim mm As String
    Dim dd As String
    ...
    Dim rtn As Long
    rtn = 一括ディレクトリ作成2(root, yyyy, mm, mm & dd)
    file_pass = root & "\" & yyyy & "\" & mm & "\" & mm & dd
    
    一括ディレクトリ作成コピー先 = file_pass
End Function

Function 一括ディレクトリ作成2(ParamArray arg()) As Long
