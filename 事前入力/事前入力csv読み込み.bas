...
Function 事前入力_読み込みmain(Customer_name As Variant)
    ...
    ' ship_date = Sheets("ピッキング表").Range("A1")  '出荷日
    ...
    MsgBox "読み込み完了しました。" & vbCrLf & vbCrLf & "【出荷先】" & Customer_name & vbCrLf & "【出荷日】" & ship_date
...
Function 事前入力csvファイル名探索(Customer_name As Variant) As Variant
    ...
    For i = Len(sFileFullPath) To 0 Step -1
        ...
        End If
    Next
    csvFilePath = sParentFolderPath & "\ピッキング表\コープ事前入力csv\" & Customer_name & "\" & Year(ship_date) & "年\" & Right("0" & Month(ship_date), 2) & "月"
...
Function 事前入力ディレクトリ作成コピー先() As Variant
    ...
    ship_date = Workbooks("生産表.xlsm").Sheets("ピッキング表").Range("D6")
