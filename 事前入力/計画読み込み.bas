...
Function 計画_C列商品貼付け(csv_data As Variant)
    'アクティブ
    Worksheets("ピッキング表").Activate
    Worksheets("ピッキング表").Select
    ship_date = Str(Sheets("ピッキング表").Range("D6"))  '出荷日
    
    '出荷日の商品数をカウント
    C_MAX = 0
    For i = 0 To UBound(csv_data)
        If csv_data(i, 2) = ship_date Then
            C_MAX = C_MAX + 1
        End If
    Next i
    ...
    ReDim C_column(0 To C_MAX - 1, 0 To 0)

    s = 0
    '商品一覧作成
    For i = 0 To UBound(csv_data)
        If csv_data(i, 2) = ship_date Then
            C_column(s, 0) = csv_data(i, 0)
            s = s + 1
        End If
    Next i
...
'  max_n = CreateObject("Scripting.FileSystemObject").OpenTextFile(file, 8).Line 'ファイルの行数取得
  ...
  ReDim ary(max_n, 3) As Variant '取得した行数で2次元配列の再定義
