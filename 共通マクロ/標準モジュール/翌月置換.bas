Attribute VB_Name = "翌月置換"
Sub 置換main()
    'セッティング 」」」」」
    Set dateList = 日付クラス()
    Set fileList = ファイルクラス()
    Set colorList = カラークラス()

    Call 実行前チェック(dateList, fileList)
    Call 全シート循環(dateList, fileList, colorList)
...
'    End If
    ...
    If fileList.mybook_month = Month(dateList.date_next) Then
        ...
        End If
    Else
        MsgBox ("①ファイル名の月" & vbCrLf & "②「合計金額」シート「B2」セル" & vbCrLf & "の関係がおかしいです。" & vbCrLf & vbCrLf & "正しい例" & vbCrLf & "①在庫『包材』_2020.8.xlsm" & vbCrLf & "②2020/7/1")
        End
    End If
End Sub

Sub 全シート循環(dateList As Variant, fileList As Variant, colorList As Variant)
    'カラー指定用配列変数の宣言
    ...
    For ws_num = 1 To (Sheets("合計金額").Index - 1)
        ...
        If UBound(varResult) <> -1 Then
            Debug.Print "カラー指定シート__" & ActiveSheet.Name
            Call カラー指定(colorGenArray)
        End If
...
Sub H_AL列置換削除(dateList As Variant, fileList As Variant)
        ...
        '入荷数の削除
        If ActiveSheet.Name <> "服部コーヒーフーズ㈱" Then
            '数式が1日～７日に入っているかどうか
            If Mid(tikan_cell(s, 4), 1, 1) = "=" And Mid(tikan_cell(s, 5), 1, 1) = "=" And Mid(tikan_cell(s, 6), 1, 1) = "=" And Mid(tikan_cell(s, 7), 1, 1) = "=" And Mid(tikan_cell(s, 8), 1, 1) = "=" And Mid(tikan_cell(s, 9), 1, 1) = "=" And Mid(tikan_cell(s, 10), 1, 1) = "=" Then
                Debug.Print ActiveSheet.Name & s & "行 " & "入荷数に数式あるので削除しない"
            Else
        ...
        End If
            ...
            'ダンボール、出荷数の削除
            If ActiveSheet.Name = "高山包装㈱" Then
                If tikan_cell(s, 1) = "出荷数(手入力)" Then
                    '数式が入っているかどうか
                    If Mid(tikan_cell(s, 4), 1, 1) = "=" Then
                        Debug.Print ActiveSheet.Name & s & "行 " & "出荷数(手入力)に数式あるので削除しない"
                    Else
                        Debug.Print ActiveSheet.Name & s & "行 " & "出荷数(手入力)削除"
                        For n = 4 To 34
                            tikan_cell(s, n) = ""
                        Next n
                    End If
...
Sub カラー指定(colorGenArray As Variant)
    '最終行取得
    Dim LastRow As Long
    ...
    'C列が４文字　かつ　E列「入荷数」or「合計入荷数」 かつ カラー指定原料コード一致
    For s = 1 To LastRow
        If Len(tikan_cell(s, 1)) = 4 Then
            If tikan_cell(s, 3) = "入荷数" Or tikan_cell(s, 3) = "合計入荷数" Then
                'Filter()関数により要素を検索
                varResult = Filter(colorGenArray, tikan_cell(s, 1))
                
                'カラー指定原料コードが一致した場合、AN列色コピペ
                If UBound(varResult) <> -1 Then
                    '以前の2020.09.10
'                                Cells(s, 39).Select 'selectセルのリセット
'                                Cells(s, 39).Copy   '入荷数の合計セル(AM4)コピー (理由はReFAX等色変更しないセルだから）
'                                Range(Cells(s, 8), Cells(s, 39)).Select '１日～３１日までセレクト
'
'                            'ペースト
'                                Selection.PasteSpecial Paste:=xlPasteFormats, Operation:=xlNone, _
'                                SkipBlanks:=False, Transpose:=False
                        rgb_value = Cells(s, 40).Interior.Color 'ANの色を取得
                        Range(Cells(s, 8), Cells(s, 39)).Interior.Color = rgb_value
                    
                    Debug.Print "カラー指定___" & tikan_cell(s, 1) & tikan_cell(s, 3)
                End If
            End If
        End If
    Next s
...
Public Function ファイルクラス() As fileClass
    ...
    date_next = DateAdd("m", 1, DateSerial(Year(date_now), Month(date_now), 1))       '翌月date
'    If Month(date_now) = 12 Then '12月の場合
'        s = InStr(fileList.mybook, Year(date_next) & ".") + 5  '「2020.」は何文字目から始まるか
'    Else                        '1～11月の場合
...
'    If Month(date_now) = 12 Then '12月
'        Fname = InStr(fileList.mybook, Year(date_next) & ".") '「2020.」は何文字目から始まるか
'    Else                        '1～11月
'        Fname = InStr(fileList.mybook, Year(date_now) & ".") '「2020.」は何文字目から始まるか
'    End If
...
Public Function カラークラス() As colorClass
    '宣言
